...
extern struct Library *SysBase, *DOSBase, *IntuitionBase, *MultimediaBase, *CharsetsBase;
...
STRPTR StrNewUTF8(STRPTR src, ULONG src_mib)
{
	...
	if(src)
	{
		if((size = GetByteSize(src, -1, src_mib, MIBENUM_UTF_8)) != -1)
		{
			STRPTR result;

			if((result = AllocVec(size, MEMF_PUBLIC)))
			{
				if(ConvertTags(src, -1, result, size, src_mib, MIBENUM_UTF_8, TAG_END) != -1)
					return result;
