 ...
 *
 *  NAME
 *    sha1.c -- Funkcje wyznaczania skrótu SHA-1
 *
 *  AUTHOR
 *    Steve Reid
 *
 ...
 *
 *  FUNCTION
 *    Oblicza skrót sha-1 dla podanego has³a i ziarna.
 *
 *  INPUTS
 *    - password -- has³o
 *    - seed -- ziarno
 *    - result -- wska¼nik na bufor na wynik (64 bajty)
 *
 *****/
...
VOID GGSha1Hash(CONST_STRPTR password, ULONG seed, UBYTE *result)
{
	...
	SHA1_Init(&ctx);
	SHA1_Update(&ctx, (const unsigned char*) password, StrLen((STRPTR)password));
